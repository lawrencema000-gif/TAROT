...
/**
 * NO HALO. `primary` and `gold` used to carry a gold box-shadow that grew on
 * hover — the "one glow per screen, on the call to action" rule. On a
 * near-black ground a solid gold fill is already the brightest object on the
 * screen; a bloom around it did not make it more clickable, it made every
 * screen look lit from behind, and it was the last coloured shadow in the
 * primitives after the cards lost theirs. Emphasis is fill now: gold is the
 * primary, everything else is quieter.
 *
...
// Transform and colour only — never a layout property, and no shadow, because
// nothing here has one.
const TRANSITION =
  `transition-[transform,background-color,border-color,color,opacity] duration-fast ${EASE_OUT}`;
...
const variants: Record<ButtonVariant, string> = {
  primary:
    'bg-gold text-mystic-950 active:bg-gold-dark ' +
  ...
  // dip in opacity and the hover as the same 2px lift a card gets — both
  // compositor-only, unlike a filter.
  gold:
    'bg-gradient-to-r from-gold-dark via-gold to-gold-light text-mystic-950 active:opacity-90 ' +
...
// Three heights, baked in, on the role type scale.
//
...
// is 48 and sm is 40 for genuinely secondary controls. The labels sit on the
// same scale as everything else (meta 13 / ui 15 / body 17) rather than
// Tailwind's unnamed 14/16/18.
